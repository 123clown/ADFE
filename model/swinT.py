...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.downsample is not None:
            y = self.downsample(y, H, W)
            H, W = (H + 1) // 2, (W + 1) // 2

        return x, y, H, W   #y是每个模块最终下采样后的结果，x是没有下采样的输出
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # self.head = nn.Linear(self.num_features, 1024)
...
def load_swin_base_patch4_window7_224(dump_path):
    model = swin_base_patch4_window7_224()
    model_dict = model.state_dict()
    pretrained_dict=torch.load(dump_path)['model']
...
def load_swin_base_patch4_window12_384(dump_path):
